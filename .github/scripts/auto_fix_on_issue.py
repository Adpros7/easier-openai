...
def codex_suggest_patch(title: str, body: str) -> str:
    print(
        f"[INFO] Asking Codex/OpenAI for patch. Title: {title!r}, body length: {len(body)}"
    )
    ...
    try:
        ...
        print("[ERROR] Exception calling OpenAI:", file=sys.stderr)
        traceback.print_exc()
        patch = ""
    print(f"[INFO] Received patch (first 200 chars): {patch[:200]!r}...")
    return patch


def apply_patch(patch_text: str) -> bool:
    if not patch_text.strip():
        print("[WARN] Patch text empty or whitespace only.")
        return False
    with tempfile.NamedTemporaryFile(mode="w+", delete=False) as tf:
        tf.write(patch_text)
        tf.flush()
        path = tf.name
    print(f"[INFO] Applying patch file at {path}")
    run_shell(f"git apply --reject --whitespace=fix {path}")
    changed = bool(run_shell("git diff --name-only").strip())
    print("[INFO] git diff names:", run_shell("git diff --name-only"))
    return changed


def commit_changes(branch: str):
    print(f"[INFO] Creating branch {branch}")
...
def main():
    ...
    print(f"[INFO] Starting auto-fix: issue #{issue_number}, title {issue_title!r}")

    if not issue_number:
        print("[WARN] ISSUE_NUMBER env var missing. Exiting.")
        sys.exit(0)

    patch = codex_suggest_patch(issue_title, issue_body)
    if not patch.strip():
        print("[INFO] No patch suggested. Exiting without PR.")
        sys.exit(0)
    ...
    # Use the new recommended way to set output
    # GitHub Actions now uses GITHUB_OUTPUT file
